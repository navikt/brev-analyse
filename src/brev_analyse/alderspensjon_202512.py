# %%
"""
Analyse av brev om alderspensjon jul 2025
"""
...
datasett_sti = "../../data/alderspensjon_202512.pkl"
...
"""
# %%
# I undersøkelsen om alderspensjon er aldersspørsmålet byttet ut med fødselsår knyttet til regelverksendringer
# %%
"""
...
# Log regresjon
# Kontaktet de Nav og morsmål
model = logit("dep ~ C(Fødselsår)", data=reg_df)
res = model.fit()
...
# %%
# Log regresjon
# Kontaktet Nav Fødselsår og morsmål
model = logit("dep ~ + C(Fødselsår) + C(Morsmål)", data=reg_df)
res = model.fit()
print("Modell nummer 3")
print(f"Formel: {res.model.formula} \n \n")
print(res.summary())
# %%
# Likelihood test for å se om Fødselsår forklarer mer enn nullhypotese - ingen variabel
# Lag begge modellene og sammenlign fit
model_with = logit("dep ~ C(Fødselsår)", data=reg_df).fit()
...
# %%
# Log regresjon
# Kontaktet Nav og Fødselsår og morsmål
model = logit("dep ~ C(Morsmål) + C(Fødselsår)", data=reg_df)
res = model.fit()
...
reg_df["dep"] = reg_df["dep"].astype(int)
# %%
# Log regresjon - forstår begrunnelse og morsmål Fødselsår og kontaktet Nav
model = logit("dep ~ C(Morsmål) + C(Fødselsår) + C(Kontaktet_Nav)", data=reg_df)
res = model.fit()
...
model = OrderedModel.from_formula(
    "dep_forstå ~ C(Fødselsår) + C(Språket_brevet) + C(Fødselsår):C(Språket_brevet)",
    distr="logit",
    data=reg_df,
)
...
model = OrderedModel.from_formula(
    "dep_forstå ~ C(Brevtype) + C(Fødselsår)",
    distr="logit",
    data=reg_df,
)
res = model.fit(method="bfgs", disp=False)
...
# %%
model = OrderedModel.from_formula(
    "dep_forstå ~ C(Brevtype) + C(Fødselsår) + C(Morsmål) + C(Fødselsår):C(Morsmål)",
    distr="logit",
    data=reg_df,
)
res = model.fit(method="bfgs", disp=False)
...
# %%
# forståelse av begrunnelse og Fødselsår
model = OrderedModel.from_formula(
    "dep_forstå ~ C(brev) + C(Fødselsår)", distr="logit", data=reg_df
)
res = model.fit(method="bfgs", disp=False)
...
print(res.summary())
# %%
# forståelse av begrunnelse og Fødselsår og morsmål
model = OrderedModel.from_formula(
    "dep_forstå ~ C(brev) + C(Fødselsår) + C(Morsmål)", distr="logit", data=reg_df
)
res = model.fit(method="bfgs", disp=False)
...
# %%
tabell.to_excel("../../data/tabell_alderspensjon_202512.xlsx", index=False)
# %%
_ = df.groupby(["Brevtype", "Antall_ganger","Tidsbruk"]).agg({"id":"count"})
_.reset_index(inplace=True)
_.to_excel("../../data/tidsbruk_alderspensjon_202512.xlsx", index=False)
# %%
