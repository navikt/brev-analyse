# %%
"""
Analyse av brev om barnetrygd jul 2025
"""
...
datasett_sti = "../../data/barnetrygd_202512.pkl"
df = pd.read_pickle(datasett_sti)
# %%
# Dropp rader fra
# De som ikke leste brevet - ingen oppfølgingsspørsmål
# De som svarte at brevet de fikk var "Jeg husker ikke" - ingen oppfølgingsspørsmål
df = df[df["Har_lest"] == "Ja"]
df = df[df["Brevtype"] != "Jeg husker ikke"]
# %%
# Dropp ubrukte kategorier
df["Har_lest"] = df["Har_lest"].cat.remove_categories(["Nei", "Unknown"])
df["Brevtype"] = df["Brevtype"].cat.remove_categories(["Jeg husker ikke"])
# %%
"""
...
# %%
tabell.to_excel("../../data/tabell_barnetrygd_202512.xlsx", index=False)
# %%
_ = df.groupby(["Brevtype", "Antall_ganger","Tidsbruk"]).agg({"id":"count"})
_.reset_index(inplace=True)
_.to_excel("../../data/tidsbruk_barnetrygd_202512.xlsx", index=False)
# %%
