...
# %%
def get_survey_questions(dataframe: pd.DataFrame) -> dict:
    """
...
# %%
def return_open_answers(dataframe: pd.DataFrame) -> list:
    """
    ...
    Returns
    -------
    openended: list of all questions containing open ended answers
    """
    df = dataframe
...
# %%
def label_questions(questions) -> list:
    """
    Get questions keys and values from survey question dictionary.

    Labels duplicate questions by appending with an increment

    Returns
    -------
    questions_inc: list of all questions in survey
    """
    questions_inc = [
